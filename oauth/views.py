...
from jungblog.blog_signals import oauth_user_login_signal
from jungblog.utils import get_current_site
from jungblog.utils import send_email, get_sha256
...
def get_redirecturl(request):
    ...
    if p.comloc:
        ...
        if not p.comloc.replace('www.', '') == site.replace('www.', ''):
...
def authorize(request):
    ...
    if user:
        if not user.nikename or not user.nikename.strip():
            user.nikename = "jungblog" + timezone.now().strftime('%y%m%d%I%M%S')
        try:
                ...
                if not author:
                    ...
                    if result[1]:
                        try:
                            ...
                            author.username = "jungblog" + timezone.now().strftime('%y%m%d%I%M%S')
    ...
    with transaction.atomic():
        if oauthuser.author:
            ...
            if result[1]:
                ...
                ) else "jungblog" + timezone.now().strftime('%y%m%d%I%M%S')
                author.save()
